...
let poly () = 
  ...
  let interval = 0--9 in
  let tasksAndFeatures = 
    List.concat @@ List.concat (List.map ~f:(fun a -> (List.map ~f:(fun b ->
	(List.map ~f:(fun c -> make_polynomial_task a b c)
    interval)) interval)) interval) in
  let tasks = List.map ~f:fst tasksAndFeatures in
  let features = List.map ~f:snd tasksAndFeatures in
  let grammar = 
    encapsulatedEC [c_S;c_B;c_C;c_I;c_plus;c_times;c_zero;c_one;] (* primitives *)
      tasks
      4 (* iterations *)
      ~lambda:1.5 (* lambda *)
  ...
  in
  train_recognition_model frontier_size grammar tasks features
;;
